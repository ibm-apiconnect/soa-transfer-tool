...
var logger=require("../lib/Logger"), Promise = require('bluebird'), fs = require('fs'), AdmZip = require("adm-zip"),
...
function storeWSDL(businessServiceBsrURI, serviceVersionBsrURI, data) {
	...
	var promise = _makeFolders(businessServiceBsrURI, serviceVersionBsrURI).then(function(){
		...
		for(var i = 0; i < data.length; i++) {
			...
			zip.addFile(location, data[i].content, data[i].name, 4);
		}
		...
		var fsPromise = fs.writeFileAsync(zipPath, buffer);
